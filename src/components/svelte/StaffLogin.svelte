<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    if (authError) { error = t('staff_login.invalid'); loading = false; return; }
    if (!data.user) { error = t('staff_login.invalid'); loading = false; return; }
    ...
    loading = false;
    if (!res.ok) { error = t('staff_login.no_staff'); await supabase.auth.signOut(); return; }

    const rows = await res.json();
    if (!rows?.length) { error = t('staff_login.no_staff'); await supabase.auth.signOut(); return; }
...
<div class="w-full max-w-sm">
  <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-lg">
    <h1 class="text-xl font-bold text-gray-900 mb-6 text-center">{t('staff_login.title')}</h1>
    <form onsubmit={handleSubmit} class="space-y-4">
      <input type="email" placeholder={t('staff_login.email')} bind:value={email} required
      ...
      <div class="flex gap-2 items-start">
        <input type={showPw ? 'text' : 'password'} placeholder={t('staff_login.password')} bind:value={password} required
        ...
        <button type="button" onclick={() => showPw = !showPw} class="p-2.5 rounded-xl border border-gray-200 hover:bg-gray-100 transition-colors text-gray-500 shrink-0">
          {#if showPw}
      ...
      <button type="submit" disabled={loading}
        ...
        {loading ? t('staff_login.logging_in') : t('staff_login.login_btn')}
      </button>
    </form>
  </div>
