  ...
  {#if showInvite}
    <div class="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
      ...
      <div class="flex gap-2 items-start">
        <input bind:value={invitePassword} type={showInvitePw ? 'text' : 'password'} placeholder="Password temporanea" class="flex-1 border border-gray-200 rounded-xl px-4 py-2 text-sm" />
        <button type="button" onclick={() => showInvitePw = !showInvitePw} class="p-2 rounded-xl border border-gray-200 hover:bg-gray-100 transition-colors text-gray-500 shrink-0" title={showInvitePw ? 'Nascondi' : 'Mostra'}>
          {#if showInvitePw}
